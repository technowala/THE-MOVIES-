...
const PREDEFINED_USERS = [
    { name: "CHANDAN", password: "MESSI2010", role: "admin" },
    { name: "SUHAS", password: "S#H1980AS", role: "viewer" },
    { name: "HRUTHVIK G SHETTY", password: "HRUT@7596VIK", role: "viewer" },
    { name: "AIDEN", password: "AIDEN@5698", role: "viewer" },
    { name: "KUSHAL", password: "KUS#1459HAL", role: "viewer" },
    { name: "MANEESH", password: "CR7", role: "admin" }
];
...
window.onValue(window.usersRef, (snapshot) => {
    ...
    if (currentUser) {
        ...
        if (updatedUser) {
            ...
            // Re-render only if history changed? 
            // renderMovies() calls getFilteredMovies which uses movies array. 
            // But renderMovies also draws history.
            // Let's safe-guard re-render if needed, but usually moviesRef triggers render.
            // If only user history changes, we might want to re-render.
            renderMovies();
        }
    }
});
...
function seedUsers() {
    PREDEFINED_USERS.forEach(pUser => {
        const exists = users.some(u => u.name === pUser.name);
        if (!exists) {
            console.log("Seeding user:", pUser.name);
            window.push(window.usersRef, pUser);
        }
    });
}
